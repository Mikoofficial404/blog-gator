...
const joinFeeds = `-- name: JoinFeeds :many
SELECT feeds.name, feeds.url, users.name AS user_name
FROM feeds
INNER JOIN users ON feeds.user_id = users.id
`

type JoinFeedsRow struct {
	Name     string
	Url      string
	UserName string
}

func (q *Queries) JoinFeeds(ctx context.Context) ([]JoinFeedsRow, error) {
	rows, err := q.db.QueryContext(ctx, joinFeeds)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []JoinFeedsRow
	for rows.Next() {
		var i JoinFeedsRow
